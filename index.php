...
$pinyin = new Pinyin(); 
/* 	var_dump("哈哈");
	var_dump($pinyin->permalink("剛剛"));exit;
	var_dump($pinyin->abbr("剛剛"));exit;
 */
/*
 1 -- 自学 -- 99 总文件数
 1 -- 教程 -- 99 总文件数
...
print_r($filesArr);
if(count($filesArr)>1){file_put_contents('mysql.txt',toUTF8('INSERT INTO `ff_vod` (`vod_cid`, `vod_name`, `vod_ename`, `vod_title`, `vod_keywords`, `vod_type`, `vod_color`, `vod_actor`, `vod_director`, `vod_content`, `vod_pic`, `vod_pic_bg`, `vod_pic_slide`, `vod_area`, `vod_language`, `vod_year`, `vod_continu`, `vod_total`, `vod_isend`, `vod_addtime`, `vod_filmtime`, `vod_hits`, `vod_hits_day`, `vod_hits_week`, `vod_hits_month`, `vod_hits_lasttime`, `vod_stars`, `vod_status`, `vod_up`, `vod_down`, `vod_ispay`, `vod_price`, `vod_trysee`, `vod_play`, `vod_server`, `vod_url`, `vod_inputer`, `vod_reurl`, `vod_jumpurl`, `vod_letter`, `vod_skin`, `vod_gold`, `vod_golder`, `vod_length`, `vod_weekday`, `vod_series`, `vod_copyright`, `vod_state`, `vod_version`, `vod_tv`, `vod_douban_id`, `vod_douban_score`, `vod_scenario`, `vod_lines`, `vod_watch`, `vod_ending`, `vod_writer`, `vod_producer`, `vod_camera`, `vod_editor`, `vod_music`, `vod_art`, `vod_extend`) VALUES '.PHP_EOL), FILE_APPEND);}
for($i=0;$i<count($filesArr);$i++) {
	if (!file_exists($foler.'shipin/'.str_pad(($i+1+$addId),5,"0",STR_PAD_LEFT))) {
		mkdir($foler.'shipin/'.str_pad(($i+1+$addId),5,"0",STR_PAD_LEFT),0777,true);
        mkdir($foler.'tupian/'.str_pad(($i+1+$addId),5,"0",STR_PAD_LEFT),0777,true);
    }
	$old_name = $foler.$filesArr[$i];
	$exts = my_path_info($old_name);
    $ext_img = NULL; $arrIMG = array('.png', '.jpg', '.bmp', '.jpeg');
    foreach ($arrIMG as $value) {$is_img = $foler.$exts['filename'].$value;
    if(file_exists($is_img)) {$ext_img = $value;$old_img = $foler.$exts['filename'].$value;}}
    if(empty($ext_img)){$addId--;continue;}
	$new_name = $foler.'shipin/'.str_pad(($i+1+$addId),5,"0",STR_PAD_LEFT).'/'.str_pad(($i+1+$addId),5,"0",STR_PAD_LEFT).'.'.$exts['extension'];
	$new_img = $foler.'tupian/'.str_pad(($i+1+$addId),5,"0",STR_PAD_LEFT).'/'.str_pad(($i+1+$addId),5,"0",STR_PAD_LEFT).$ext_img;
...
function getDirFiles($folder) {
	...
	if(is_dir($folder)){
		...
		while($file = readdir($hander)){
			$path_name = substr($file, strrpos($file, '.'));
			if($file=='.'||$file=='..'||$file=='file.php'){
				continue;
			}elseif(is_file($folder.'/'.$file)&&$path_name=='.mp4'){
				$filesArr[] = $file;
			}
		}
	}
...
function pinyin($sGBK, $sRetFormat='head') {
	$aBuf = array();
...
function toUTF8($value) {
    return mb_convert_encoding(trim($value), 'utf-8', mb_detect_encoding(trim($value), array('ASCII','UTF-8','GB2312','GBK','LATIN1','BIG5')));
}
